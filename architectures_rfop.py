...
#  Voice : resemblyzer VoiceEncoder            → 512-d
...
VOICE_FEAT_DIM = 512    # RFOP checkpoint expects 512-d voice features
...
class VoiceFeatureExtractor:
    """
    Extracts 512-d voice embeddings using HuggingFace HuBERT.
    Install: pip install transformers soundfile torchaudio
    """
    def __init__(self, device: torch.device):
        self.device = device
        try:
            self.processor = Wav2Vec2FeatureExtractor.from_pretrained("facebook/hubert-base-ls960")
            self.hubert = HubertModel.from_pretrained("facebook/hubert-base-ls960").eval().to(device)
        except Exception as e:
            raise ImportError(
                "Failed to initialize HuBERT voice feature extractor. "
                "Install transformers and ensure internet access for model download. "
                f"Original error: {e}"
            )
        self.proj = nn.Sequential(
            nn.Linear(self.hubert.config.hidden_size, VOICE_FEAT_DIM),
            nn.BatchNorm1d(VOICE_FEAT_DIM),
            nn.ReLU(inplace=True),
            nn.Linear(VOICE_FEAT_DIM, VOICE_FEAT_DIM),
        ).eval()
        for m in self.proj:
    ...
    def extract(self, audio_path: str) -> np.ndarray:
        ...
        if sr != 16000:
            audio = torchaudio.functional.resample(torch.from_numpy(audio), orig_freq=sr, new_freq=16000).numpy()
