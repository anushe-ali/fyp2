...
def xception(num_classes=1000, pretrained=False):
    """
    pretrained=False  → random init (use when loading fine-tuned weights)
    pretrained='imagenet' → load imagenet weights (only valid for num_classes=1000)
    """
    model = Xception(num_classes=num_classes)

    if pretrained == 'imagenet':
        assert num_classes == 1000, \
            f"num_classes must be 1000 for imagenet pretrained, got {num_classes}"
    ...
    # rename fc → last_linear (convention used by FaceDeepfakeDetector)
